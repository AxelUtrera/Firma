...
const botonConstrato = document.querySelector('.boton-contrato');
...
botonConstrato.addEventListener('click', (e) => {
    e.preventDefault();
...
function onClickDedoLevantado() {
    modoEscritura = false;
}

['mousedown', 'touchstart'].forEach(nombreEvento => {
    canvas.addEventListener(nombreEvento, onClickToqueIniciado, {passive:true});
});
...
['mouseup', 'touchend'].forEach(nombreEvento => {
    canvas.addEventListener(nombreEvento, onClickDedoLevantado, {passive:true});
});
